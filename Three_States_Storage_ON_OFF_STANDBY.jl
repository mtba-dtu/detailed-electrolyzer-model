...
@constraint(m3s, [t=T],
    e_tot[t] == sum(e[t,s] for s=S) + P_sb * z_sb[t])
# Hydrogen production
@constraint(m3s, [t=T],
...
@constraint(m3s, [t=T],
    e_tot[t] <= C_E * z_on[t] + P_sb * z_sb[t])
# Minimum electrolyzer power
@constraint(m3s, [t=T],
    e_tot[t] >= P_min * z_on[t] + P_sb * z_sb[t])
# only one efficiency if on or standby
@constraint(m3s, [t=T],
    z_on[t] == sum(z_h[t,s] for s=S))
# States
@constraint(m3s, [t=T],
    1 == z_on[t] + z_off[t] + z_sb[t])
# Not from Off to Standby
@constraint(m3s, [t=T],
    1 >= (t > 1 ? z_sb[t] + z_off[t-1] : 0))
# Startup cost
@constraint(m3s, [t=T],
    z_start[t] >= (t > 1 ? z_on[t] - z_on[t-1] - z_sb[t-1] : 0))
...
#************************************************************************
# Output to EXCEL
XLSX.writetable("m3s_$(segments)_segments.xlsx", output, overwrite=true)
